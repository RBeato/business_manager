...
export async function publishBlogPost(
  ...
  const db = getDb()
  const post = db.prepare('SELECT * FROM blog_posts WHERE id = ?').get(blogPostId) as any
  ...
  if (typeof post.keywords === 'string') {
    try { post.keywords = JSON.parse(post.keywords) } catch { post.keywords = [] }
  }
  ...
  db.prepare('UPDATE blog_posts SET status = ?, published_date = ? WHERE id = ?')
    .run(
      options.autoMerge ? 'published' : 'approved',
      options.autoMerge ? new Date().toISOString() : null,
      blogPostId
    )
...
export async function publishScheduledPosts() {
  ...
  const posts = db.prepare(
    "SELECT * FROM blog_posts WHERE status = 'approved' AND scheduled_publish_date IS NOT NULL AND scheduled_publish_date <= ?"
  ).all(now) as any[]
